...
int init_hashtables()
{
    ...
    inode_hash_table = calloc(id_table_size, sizeof(struct notifyfs_entry_struct *));
...
{
    struct notifyfs_inode_struct *tmpinode = inode_hash_table[inode_2_hash(inode)];
...
static struct notifyfs_inode_struct *create_inode()
{
	...
	inode->ino = inoctr;
	inode->nlookup = 0;
	inode->status=NOTIFYFS_INODE_STATUS_OK;
...
int determine_path(struct call_info_struct *call_info, unsigned char flags)
{
    ...
    *pathstart = '\0';
    ...
    if ( flags & NOTIFYFS_PATH_BACKEND ) {

        if ( call_info->typebackend==BACKEND_METHOD_NOTSET ) {

            if ( tmpentry->inode ) {

                if ( tmpentry->inode->effective_watch ) call_info->typebackend=tmpentry->inode->effective_watch->typebackend;
	...
	*pathstart = '/';
    ...
    /* set default backend when not set */

    if ( flags & NOTIFYFS_PATH_BACKEND ) {

        if ( call_info->typebackend==BACKEND_METHOD_NOTSET ) call_info->typebackend=BACKEND_METHOD_INOTIFY;
...
{
    effective_watch->typebackend=BACKEND_METHOD_INOTIFY; /* default */
	...
	if ( mount_entry->client ) {
	    ...
	    effective_watch->typebackend=BACKEND_METHOD_FORWARD;
...
struct effective_watch_struct *lookup_watch(unsigned char type, unsigned long id)
{
    ...
    while (effective_watch) {
        ...
        if ( effective_watch->id==id && effective_watch->typebackend==type ) break;
