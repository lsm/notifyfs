...
{
    unsigned long int seca;
    unsigned long int nseca;
    unsigned char extra=0;

    nseca=ats->tv_nsec+nsec;
    ...
    seca=ats->tv_sec+sec+extra;

    if ( seca > bts->tv_sec) {

	return 1;

    } else if ( seca==bts->tv_sec) {

	if ( nseca > bts->tv_nsec ) {

	    return 1;

	} else if (nseca == bts->tv_nsec ) {
